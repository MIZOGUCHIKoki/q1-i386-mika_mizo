...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[black,thick,mark=*] table[x index=0,y index=1,col sep=comma]{exdataA.csv};
            \addlegendentry{アセンブリ言語}
            \addplot[black,thick,dashed,mark=square*,mark options={solid}]table[x index=0,y index=1, col sep=comma]{exdataJ.csv};
...
ただし，いずれも\ref{src:sort.java}，\ref{src:sort.s}のアルゴリズムでの実行時間であるので，一般的にアセンブリ言語の方が{\ttfamily Java}よりも実行時間が短いとの\par
