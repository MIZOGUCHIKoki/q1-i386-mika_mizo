...
\begin{minipage}[t]{0.6\textwidth}
    ...
    \begin{tikzpicture}[scale=0.9]
        ...
        info{
                % \draw plot(\x, {7 * pow(10,-10) * pow(\x,2) + 2 * pow(10,-5)\x - 0.0236});
            }
    \end{tikzpicture}
\end{minipage}
\section{考察}
\(n\)をデータ個数，\(t\)を実行時間，\(c\)を定数とする．\par
実験結果より，データの個数が増すと実行時間は一定ではなく増加する．また，\(t=cn\)の比例関数でないことも明らかである．\par
入力サイズ\(n\)に対してのアルゴリズムの計算量で用いられる代表的な関数の漸近的な大小関係は，
\[\log n<\sqrt{n}<n<n\log n<n^2<n^3<\dots<2^n<n!\]
であること\cite[p.7]{アルゴリズムとデータ構造}と実験結果を元に考えると，このアルゴリズムの計算量は\(O(n\log n)\)以上であることがわかる．ただし，実験結果を見る限り計算量が\(O(n^2)\)であると断定できない．
